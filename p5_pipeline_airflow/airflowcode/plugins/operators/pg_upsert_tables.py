...
class PgUpsertOperator(BaseOperator):
    """
    Demonstrator of an Operator to UPSERT data in PG, and then perform data quality checks
    ...
    Notes:
        - This Postgres version uses SELECT DISTINCT ON (not supported by Redshift)
    """
    ui_color = '#F98866'
    q_temp_drop = """DROP TABLE IF EXISTS {stage};"""
    q_temp_create = """CREATE TEMP TABLE IF NOT EXISTS {stage}  (LIKE {table});"""
    q_temp_load = """
    INSERT INTO {stage}
    {query};
    """
    ...
    q_insert_target = """INSERT INTO {table} SELECT DISTINCT ON({pkey}) * FROM {stage};"""
    ...
    def __init__(self,
        ...
        self.qf_all = [S.SQL(q).format(**self.params) for q in PgUpsertOperator.q_all]
        self.qf_rowcount = S.SQL(PgUpsertOperator.q_rowcount).format(**self.params)
        self.qf_pkeycount = S.SQL(PgUpsertOperator.q_pkeycount).format(**self.params)

    def execute(self, context):
        """
        Uses a PostgresHook to connect to a postgre database (like Redshift) and execute an UPSERT
        Also perform data quality checks
        Args:
